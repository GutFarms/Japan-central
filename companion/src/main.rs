//! CYD Companion — GPU desktop app (USB control + network data bridge) for the CYD miner.
//! Fetches market/network data on the PC and pushes it to the board via `cmp netdata`.
//!
//! ```text
...
Board mines at 240 MHz with hash-focus after setup.",
    ...
    fn ui_markets(&mut self, ui: &mut egui::Ui) {
        ...
        ui.label("Five coins · CoinGecko on this PC · pushed to the board over USB (`cmp netdata`) every 30s.");
        ui.add_space(10.0);
        if !self.market_error.is_empty() {
            ui.colored_label(Color32::from_rgb(220, 130, 110), &self.market_error);
            ui.add_space(6.0);
...
&vs_currencies=usd&include_24hr_change=true&include_market_cap=true&include_24hr_vol=true"
                ...
                // Push PC-fetched market/network data to the board over USB.
                if let Ok(ref quotes) = r {
                    if let Some(p) = usb.as_mut() {
                        let body = encode_netdata_push(quotes);
                        match usb_cmd(p.as_mut(), &mut usb_rx, &format!("cmp netdata {body}")) {
                            Ok(_) => {
                                let _ = msg_tx.send(NetMsg::Action(Ok(
                                    "USB ← network data pushed to board".into(),
                                )));
                            }
                            Err(e) => {
                                let _ = msg_tx.send(NetMsg::Action(Err(format!(
                                    "USB netdata push failed: {e}"
                                ))));
                            }
                        }
                    }
...
/// Build `cmp netdata …` body: PC-fetched network/market line for the board LCD.
fn encode_netdata_push(quotes: &[CoinQuote]) -> String {
    let mut parts = Vec::new();
    for q in quotes.iter().take(5) {
        if q.price_usd <= 0.0 {
            continue;
        }
        let sign = if q.change_24h >= 0.0 { '+' } else { '-' };
        parts.push(format!(
            "{} {}{}{:.1}%",
            q.symbol,
            format_usd(q.price_usd),
            sign,
            q.change_24h.abs()
        ));
    }
    ...
    // Keep UART line short for the board RX buffer.
    let text = if text.len() > 90 { text[..90].to_string() } else { text };
    format!(
        "source=coingecko&text={}",
        urlencode_form(&text)
    )
}
