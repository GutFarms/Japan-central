...
The cloud agent’s `cursor[bot]` token can push to `Japan-central` only. Product repos were pushed with a local GitHub login.

**Native Pure is intentionally not pushed** — it stays on Japan-central (`cursor/split-to-native-repo-2cb7` / `split/native`).

```bash
git clone https://github.com/GutFarms/Japan-central.git
cd Japan-central
git fetch origin \
  split/farm-manager split/boriken-llm split/cyd-miner \
